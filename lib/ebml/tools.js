...
export const readVint = _tools.readVint;
export const writeVint = _tools.writeVint;
export const ebmlBlock = _block;
...
/**
  ...
  */
export function encodeTag(tagId, tagData, unknownSize = false) {
    return concat([
        tagId,
        unknownSize ?
 ...
 */
export function WebPBlockFilter(elms) {
 ...
 */
export function VP8BitStreamToRiffWebPBuffer(frame) {
 ...
 * RIFF データチャンクを作る
 */
export function createRIFFChunk(FourCC, chunk) {
 ...
 */
export function makeMetadataSeekable(originalMetadata, duration, cuesInfo) {
    ...
    let info = extractElement("Info", originalMetadata);
    ...
    let finalMetadata = [].concat.apply([], [
        header,
 ...
 */
export function removeElement(idName, metadata) {
    let result = [];
    let start = -1;
    for (let i = 0; i < metadata.length; i++) {
 ...
 * @param metadata - array of EBML elements to search
 */
export function extractElement(idName, metadata) {
    let result = [];
    let start = -1;
    for (let i = 0; i < metadata.length; i++) {
        let element = metadata[i];
 ...
 */
export function putRefinedMetaData(metadata, info) {
    ...
    for (i = 1; i < 20; i++) {
        ...
        if (newNewRefinedSize === newPayloadSize) {
            // Size is stable
            return new Encoder().encode([].concat(ebml, [{ type: "m", name: "Segment", isEnd: false, unknownSize: true }], refined));
        }
...
function encodedSizeOfEbml(refinedMetaData) {
    ...
    return refinedMetaData.reduce((lst, elm) => lst.concat(encorder.encode([elm])), []).reduce((o, buf) => o + buf.byteLength, 0);
}
function refineMetadata(mesetadata, sizeDiff, info) {
    const { duration, clusterPtrs, cues } = info;
    let _metadata = mesetadata.slice(0);
...
function create_seekhead(metadata, sizeDiff) {
    ...
    ["Info", "Tracks", "Cues"].forEach((tagName) => {
        ...
        switch (tagName) {
            case "Info":
                seeks.push({ name: "SeekID", type: "b", data: new Buffer([0x15, 0x49, 0xA9, 0x66]) });
                break;
            case "Tracks":
                seeks.push({ name: "SeekID", type: "b", data: new Buffer([0x16, 0x54, 0xAE, 0x6B]) });
                break;
            case "Cues":
                seeks.push({ name: "SeekID", type: "b", data: new Buffer([0x1C, 0x53, 0xBB, 0x6B]) });
                break;
        }
...
function insertTag(_metadata, tagName, children, insertHead = false) {
    ...
    if (idx >= 0) {
        ...
        [].concat([{ name: tagName, type: "m", isEnd: false }], children, [{ name: tagName, type: "m", isEnd: true }]).reverse().forEach((elm) => { _metadata.unshift(elm); });
    }
    else {
...
export function encodeValueToBuffer(elm) {
...
export function createUIntBuffer(value) {
...
export function createIntBuffer(value) {
...
export function createFloatBuffer(value, bytes = 8) {
...
export function convertEBMLDateToJSDate(int64str) {
